function calculateInterest() {
  ...
  const resultElement = document.getElementById("result");
  resultElement.innerHTML = `The interest between ${startDate} and ${endDate} is: <h3>${interest.toFixed(
    2
  )}</h3> 
            Duration: <b>${years}</b> years,<b> ${months} </b>months, <b>${days}</b> days <br> 
            Total Amount (including interest) is:<h3>${totalAmount.toFixed(
              2
            )} </h3>`;
  return false;
}
